...
def main(case, user, password):
    ...
    for date in dates:
        ...
        file1 = f'3B-HHR-L.MS.MRG.3IMERG.{datetime.strftime(date, "%Y%m%d-S%H%M%S-E%H2959")}.{str(minutes_from_date_0h).zfill(4)}.V06{type_product}.HDF5'
        file2 = f'3B-HHR-L.MS.MRG.3IMERG.{datetime.strftime(date + timedelta(minutes = 30), "%Y%m%d-S%H%M%S-E%H5959")}.{str(minutes_from_date_0h + 30).zfill(4)}.V06{type_product}.HDF5'
        print(f'downloading files: {[file1, file2]}')
        os.system(f'wget --no-check-certificate --user {user} --password {password} https://gpm1.gesdisc.eosdis.nasa.gov/data/GPM_L3/GPM_3IMERGHHL.06/{date.year}/{day_of_year}/{file1}')
        os.system(f'wget --no-check-certificate --user {user} --password {password} https://gpm1.gesdisc.eosdis.nasa.gov/data/GPM_L3/GPM_3IMERGHHL.06/{date.year}/{day_of_year}/{file2}')
        # postprocess raw files
        values_intrahour = []
        for file in (file1, file2):
            values_pcp_raw = get_vars_from_IMERG(file1, [obs_var_get,])
